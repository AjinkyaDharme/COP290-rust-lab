//! Recalculation module for the spreadsheet application.
//!
//! This module handles dependency tracking and recalculation of cells in the spreadsheet.
//! It provides:
//! - Dependency graph management
//! - Cycle detection in cell references
//! - Topological sorting for efficient recalculation
//! - Range dependency tracking
//! - Conversion between cell references and string representations
...
/// Represents a range of cells from a start to end position.
///
/// Used for tracking dependencies when a cell uses a range function like SUM or AVG.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct RangeRef {
    /// The starting cell reference of the range.
    pub start: CellRef,
    /// The ending cell reference of the range.
    pub end: CellRef,
}

impl RangeRef {
    /// Checks if a cell is contained within this range.
    ///
    /// # Arguments
...
/// Converts a cell reference string (e.g., "A1") to a CellRef object.
///
/// # Arguments
/// * `s` - The string representation of a cell (e.g., "A1", "BC23")
///
/// # Returns
/// * `Some(CellRef)` if the string can be parsed successfully
/// * `None` if the string is not a valid cell reference
...
/// Type alias for a dependency graph mapping cell keys to sets of dependent cells.
pub type DependencyGraph = HashMap<String, HashSet<String>>;

/// Manages dependencies between cells and handles recalculation ordering.
///
/// This struct tracks:
/// - Direct cell-to-cell dependencies
/// - Range dependencies (when cells are referenced through ranges)
/// - Parent-child relationships for updating affected cells
#[derive(Debug, Clone)]
pub struct RecalcManager {
    /// Maps a cell to all cells that depend on it (children)
    pub children: DependencyGraph,
    /// Maps a cell to all cells it depends on (parents)
    pub parents: DependencyGraph,
    /// Maps a cell to all ranges it depends on
    pub range_parents: HashMap<String, HashSet<RangeRef>>,
    /// All range dependencies with their dependent cells
...
impl RecalcManager {
    /// Creates a new, empty RecalcManager.
    ///
    /// # Returns
    /// A new RecalcManager with empty dependency graphs.
    pub fn new() -> Self {
        Self {
    ...
    /// Updates the dependency graph for a command and computes affected cells.
    ///
    /// # Arguments
    /// * `cmd` - The command to process (only SetCell commands modify the dependency graph)
    ///
    /// # Returns
    /// * `Ok(Vec<String>)` - A topological ordering of cells that need to be recalculated
    /// * `Err(String)` - An error message if a dependency cycle is detected
    ...
    /// Performs a topological sort to find the order of cell recalculation.
    ///
    /// # Arguments
    /// * `exclude` - The cell reference to exclude from the sort (usually the cell being modified)
    ///
    /// # Returns
    /// * `Ok(Vec<String>)` - A topological ordering of cells that need to be recalculated
    /// * `Err(String)` - An error message if a dependency cycle is detected
...
/// Recalculates cells in the spreadsheet according to the provided topological order.
///
/// # Arguments
/// * `sheet` - The spreadsheet to update
/// * `topo_order` - The topological ordering of cells that need recalculation
...
/// * `refs` - HashSet to collect the cell references found in the expression
...
/// Recursively extract all range references from an expression and add them to `range_refs`.
///
/// # Arguments
/// * `expr` - The expression to analyze
/// * `range_refs` - HashSet to collect the range references found in the expression
...
/// Converts a cell reference to its string representation (e.g., A1, BC23).
///
/// # Arguments
/// * `cell` - The cell reference to convert
///
/// # Returns
/// A string representing the cell reference (e.g., "A1", "BC23")
